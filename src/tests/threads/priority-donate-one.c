...
test_priority_donate_one (void) 
{
  ...
  lock_init (&lock);
  lock_acquire (&lock);
  msg("Location priortiy-donate-one");
...
acquire1_thread_func (void *lock_) 
{
  ...
  msg("acquire 1: Trying to get the lock");
  lock_acquire (lock);
  msg ("acquire1: got the lock %d",(lock->semaphore).value);
  lock_release (lock);
  msg ("acquire1: done");
}
...
{
  struct lock *lock = lock_;
  msg("acquire 2: Trying to get the lock");
  lock_acquire (lock);
  msg ("acquire2: got the lock %d",(lock->semaphore).value);
  lock_release (lock);
  msg ("acquire2: done");
}
